...
class Renderer:
    ...
    def _draw_player_stats(self, data: RenderData, player_id: int, area_x: int) -> None:
        ...
        panel_width = min(
            max(speed_row_width, distance_width) + 24,
            self.player_width - 190,
        )
    ...
    def _load_success_sounds(self) -> Dict[int, pygame.mixer.Sound]:
        sound_dir = Path(__file__).with_name("sound")
        sounds = {}

        for player_id in range(self.player_count):
            sound_path = sound_dir / f"correct_player{player_id + 1}.mp3"
            if not sound_path.exists():
                continue

            try:
                sounds[player_id] = pygame.mixer.Sound(str(sound_path))
            except pygame.error:
                continue
